...
print("")
print("Enter the VLAN ID, the port you like to change and the new port mode (tagged or untagged)")
...
##############################################################
    ...
    ## CHANGE PORT STATUS ON VLAN
    add_port_to_vlan = requests.put(url + 'vlans-ports/' + str_vlan_id + "-" + get_port_id, data=json.dumps(vlan_port_data), headers=headers, verify=False, timeout=2)
